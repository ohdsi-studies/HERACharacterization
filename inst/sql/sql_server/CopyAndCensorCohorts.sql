...
inner join (
  ...
  INNER JOIN (SELECT DISTINCT target_id cohort_definition_id FROM #TARGET_SUBGROUP_XREF) t 
  ON t.cohort_definition_id = cs.cohort_definition_id 
  where cs.num_persons > @min_cell_count
  UNION ALL
  -- Bulk subgroup cohorts will contain only 1 entry
  -- so they must be identified by the presence of only a single
  -- cohort_type
  SELECT DISTINCT xref.cohort_id
  FROM (
  	SELECT subgroup_id, target_id, COUNT(DISTINCT cohort_type) cnt
  	FROM #TARGET_SUBGROUP_XREF
  	group by subgroup_id, target_id HAVING COUNT(DISTINCT cohort_type) = 1
  ) single
  INNER JOIN #TARGET_SUBGROUP_XREF xref ON single.subgroup_id = xref.subgroup_id
