...
cohort <- cohort %>% dplyr::distinct() %>% dplyr::select(.data$cohortFullName,
                                                         .data$cohortId,
                                                         .data$cohortName) %>%
  dplyr::arrange(.data$cohortFullName, .data$cohortId)

database <- database %>% dplyr::distinct() %>% dplyr::arrange(.data$databaseId)

if (exists("covariate")) {
  covariate <- covariate %>% dplyr::distinct() %>% dplyr::arrange(.data$covariateName)
  if (!"conceptId" %in% colnames(covariate)) {
    ...
    covariate <- covariate %>% dplyr::mutate(conceptId = (.data$covariateId - .data$covariateAnalysisId)/1000)
  }
}

if (exists("temporalCovariate")) {
  temporalCovariate <- temporalCovariate %>% dplyr::distinct() %>% dplyr::arrange(.data$covariateName,
                                                                                  .data$timeId)
  if (!"conceptId" %in% colnames(temporalCovariate)) {
    ...
    temporalCovariate <- temporalCovariate %>% dplyr::mutate(conceptId = (.data$covariateId - .data$covariateAnalysisId)/1000)
  }
  temporalCovariateChoices <- temporalCovariate %>% dplyr::select(.data$timeId,
                                                                  .data$startDayTemporalCharacterization,
                                                                  .data$endDayTemporalCharacterization) %>% dplyr::distinct() %>% dplyr::mutate(choices = paste0("Start ",
                                                                                                                                                                                                                       .data$startDayTemporalCharacterization,
                                                                                                                                                                                                                       " to end ",
                                                                                                                                                                                                                       .data$endDayTemporalCharacterization)) %>%
    dplyr::select(.data$timeId, .data$choices) %>% dplyr::arrange(.data$timeId)
}

if (exists("includedSourceConcept")) {
  conceptSets <- includedSourceConcept %>% dplyr::select(.data$cohortId,
                                                         .data$conceptSetId,
                                                         .data$conceptSetName) %>%
    dplyr::distinct() %>% dplyr::arrange(.data$cohortId, .data$conceptSetName)
} else if (exists("orphanConcept")) {
  conceptSets <- orphanConcept %>% dplyr::select(.data$cohortId,
                                                 .data$conceptSetId,
                                                 .data$conceptSetName) %>%
    dplyr::distinct() %>% dplyr::arrange(.data$cohortId, .data$conceptSetName)
} else {
...
if ("phenotypeDescription.csv" %in% list.files(path = dataFolder)) {
  ...
  cohort <- cohort %>% dplyr::rename(cohortFullNameOld = .data$cohortFullName) %>% dplyr::left_join(y = cohortDescription %>%
    dplyr::mutate(cohortId = utils::type.convert(.data$atlasId),
                  cohortFullName = .data$cohortDefinitionName) %>%
    dplyr::select(.data$cohortId,
                  .data$cohortFullName)) %>% dplyr::relocate(.data$cohortFullName) %>%
    dplyr::mutate(cohortFullName = dplyr::case_when(is.na(.data$cohortFullName) ~ .data$cohortFullNameOld,
                                                    TRUE ~ .data$cohortFullName)) %>% dplyr::select(-.data$cohortFullNameOld) %>% dplyr::arrange(.data$cohortFullName)
